"""
Train-test split (time-based), LightGBM training,
...
def train_model(
    ...
    Uses log1p transform on target.
    """
    ...
    for c in feature_cols:
        ...
        if (
            X_train[c].dtype.name == "object"
            or (
                hasattr(X_train[c].dtype, "kind")
                and X_train[c].dtype.kind == "O"
            )
        ):
